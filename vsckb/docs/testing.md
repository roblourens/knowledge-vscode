...
```
Add `--grep "<test name>"` to focus on a single test (`listModels`, `cd-prefix`, etc.) — without it the full real-SDK suite runs and takes minutes.
...
- **gotcha** (2026-04-21, protocol/toolApprovalRealSdk.integrationTest.ts) — gated on `AGENT_HOST_REAL_SDK=1` and not run by CI, so any string identifier embedded in this file (provider ids, agent names, well-known config keys) can sit broken indefinitely after a rename — TypeScript doesn't catch it (the API parameters are typed as plain `string`) and the suite never runs unattended. When renaming anything in the agent host that has a corresponding string in this file, manually run the full suite (remember to `unset ELECTRON_RUN_AS_NODE` first; see § 3 for the full invocation) and grep the file for the old name.
