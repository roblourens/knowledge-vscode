...
_Covers: src/vs/platform/agentHost/node/copilot/copilotToolDisplay.ts, src/vs/platform/agentHost/node/copilot/mapSessionEvents.ts, src/vs/platform/agentHost/node/agentEventMapper.ts, src/vs/platform/agentHost/common/commandLineHelpers.ts, src/vs/platform/agentHost/test/node/copilotToolDisplay.test.ts, src/vs/platform/agentHost/test/node/mapSessionEvents.test.ts, src/vs/platform/agentHost/test/node/agentEventMapper.test.ts, src/vs/platform/agentHost/test/common/commandLineHelpers.test.ts, extensions/copilot/src/extension/chatSessions/copilotcli/common/copilotCLITools.ts_
...
**Live path.** `agentEventMapper.ts` translates streaming `IAgentProgressEvent`s into `SessionAction`s. It tracks `_currentMarkdownPartId` and `_currentReasoningPartId` per session so that successive `delta` / `reasoning` events append to the same ResponsePart instead of creating a new one each time. The Copilot SDK emits **multiple rounds** of (reasoning → message → tool calls) within a single chat turn, so when a `tool_start` arrives, **both** part IDs must be cleared — otherwise the next round's reasoning gets appended via `SessionReasoning` onto the very first reasoning part (which sits before any `ToolCall` in `responseParts`), and on warm restore from in-process state, all reasoning renders bunched at the top of the response.
...
- **gotcha** (2026-04-28, agentEventMapper.ts:tool_start case) — the `tool_start` branch must clear **both** `_currentMarkdownPartId` AND `_currentReasoningPartId`. Forgetting the reasoning one causes every later round's reasoning in a multi-round turn to be appended onto the very first reasoning ResponsePart (via `SessionReasoning`), which sits before any `ToolCall`, so reasoning bunches at the top on warm restore. Symmetric to `_buildTurnsFromMessages` on the history-replay side.
...
- **2026-04-28** — 22c8ec60f5 — renamed "History replay and reasoning order" → "Reasoning ordering: live and history-replay paths"; documented the live-path symmetry rule (`agentEventMapper.ts` must clear both `_currentMarkdownPartId` and `_currentReasoningPartId` on `tool_start`); added `agentEventMapper.ts` to `Covers:`; added gotcha for the multi-round reasoning bunching bug.
