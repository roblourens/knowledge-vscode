...
- **gotcha** (2026-04-26, chatSessions.contribution.ts:registerChatSessionContribution + agentHostChatContribution.ts:_registerAgent) — `AgentHostContribution` (and `RemoteAgentHostContribution`) call `IChatSessionsService.registerChatSessionContribution` programmatically, which bypasses the extension-point `_evaluateAvailability` path. Two downstream listeners are gated on `_contributionDisposables.has(type)`, not on `_contributions.has(type)`: (a) the autorun that registers `workbench.action.chat.openNewChatSessionInPlace.<type>` (without it the session-type picker throws "command not found" when switching to the agent host); (b) the `chatHasCanDelegateProviders` context key. `registerChatSessionContribution` therefore has to populate `_contributionDisposables` and call `_updateHasCanDelegateProvidersContextKey()` itself on both register and dispose. If you add a third listener that also keys off `_contributionDisposables`, audit this method too.
- **gotcha** (2026-04-26, agentHostChatContribution.ts:_registerAgent + chat session picker) — In VS Code, both the agent-host Copilot CLI (`agent-host-copilotcli`) and the extension-host Copilot CLI harness contribute a session type with displayName `"Copilot CLI"`, so the picker shows two indistinguishable entries. `_registerAgent` suffixes the agent-host displayName with `" - Agent Host"` when `!_isSessionsWindow`. The Agents app keeps the original displayName (only agent-host sessions exist there). If a future agent-host provider collides with an extension-host harness name the same way, extend the suffix logic rather than renaming the upstream `agent.displayName`.
...
- **2026-04-26** — `8b0b362bd2` — added two gotchas: (1) `registerChatSessionContribution` must populate `_contributionDisposables` and update the `chatHasCanDelegateProviders` context key on register/dispose, since programmatic registrations bypass `_evaluateAvailability` (manifested as missing `openNewChatSessionInPlace.agent-host-copilotcli` command); (2) agent-host displayName is suffixed with `" - Agent Host"` in VS Code to disambiguate from the extension-host Copilot CLI harness.
