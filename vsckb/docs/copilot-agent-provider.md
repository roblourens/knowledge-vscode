...
- **gotcha** (2026-05-04, agentService.ts:_maybeEvictIdleSession) — eviction must walk the full subagent ancestry to the **root** before choosing an eviction target. All session state (turn tree, tool calls) is owned by the root; evicting only an intermediate subagent parent leaves the root session state cached indefinitely. The implementation loops `while ((parsed = parseSubagentSessionUri(evictionTarget))) evictionTarget = parsed.parentSession;` before checking subscribers.
