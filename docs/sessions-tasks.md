# Sessions tasks (agent window `inAgents` tasks.json)
...
3. **Auto-dispatch on worktree creation** — `WorktreeCreatedTaskDispatcher` (`worktreeCreatedTaskDispatcher.ts`). Workbench contribution that subscribes to `ISessionsManagementService.onDidStartSession` (fired once a session commits past `Untitled`) and tracks each started session with a per-session `autorun` that waits until `!session.loading` and the session reports an actual worktree (`session.workspace.read(reader)?.folders.some(folder => folder.gitRepository?.workTreeUri)`), then calls `_sessionsTasksService.runTask(task, session)` once per task tagged `runOptions.runOn === 'worktreeCreated'`. Sessions whose runtime already runs these tasks itself (`session.capabilities.get().runsWorktreeCreatedTasks` — capabilities is now an observable, read via `.get()`) are skipped to avoid double-execution. Dispatched task handles are tracked per session and disposed when the session is archived or removed, so long-running setup processes don't leak (#321021).
...
`WorktreeCreatedTaskDispatcher` gates auto-dispatch for **agent-host sessions only** via the `chat.agentHost.runWorktreeCreatedTasks` setting (default `true`). The gate is in the dispatcher (not the runner) on purpose: gating in the runner would also break manual `Run Task` from the toolbar, which is unaffected by the auto-dispatch setting. The check uses `isAgentHostProviderId(session.providerId)` (a pure string predicate over `LOCAL_AGENT_HOST_PROVIDER_ID` / `REMOTE_AGENT_HOST_PROVIDER_PREFIX`) — no `ISessionsProvidersService` lookup needed, because the gate is decided entirely by the well-known provider ID prefix.

## Tests

Tests live alongside the production code:

- `src/vs/sessions/contrib/chat/test/browser/worktreeCreatedTaskDispatcher.test.ts` — covers the dispatch lifecycle (dispatch on start, ignores plain `onDidChangeSessions.added` without a start event, loading flicker, waiting for untitled sessions to start, per-session teardown on removal, capability-skip, agent-host setting gate, stopping dispatched tasks on archive/removal).
...
- **gotcha** (2026-05-25, `worktreeCreatedTaskDispatcher.ts:_dispatchWorktreeCreatedTasks`) — the agent-host gate uses `isAgentHostProviderId(session.providerId)`, not `isAgentHostProvider(provider)`. Don't reintroduce an `ISessionsProvidersService` lookup here — the check is fully decidable from the well-known provider ID prefix, and looking up the provider would silently let auto-dispatch through if the provider were momentarily unregistered. Still accurate against current code.

## Changelog

- **2026-07-02** — f9f2fd558a — reconciliation: `WorktreeCreatedTaskDispatcher` was substantially rewritten since the 2026-05-25 baseline (missed by the 2026-06-25 reconciliation) — it now tracks sessions via `onDidStartSession` + a single per-session `autorun` instead of the old `_isPendingWorktreeSession` pending-untitled handoff, and reads capabilities via `session.capabilities.get().runsWorktreeCreatedTasks` (`b10844efce8` made `capabilities` observable). The `chat.agentHost.runWorktreeCreatedTasks` setting default flipped `false` → `true`. Updated the Roles/Tests sections and pruned the superseded pending-handoff gotcha and resolved setting-default debt entry.
