...
|---|---|---|---|
| `PermissionPicker` / `MobilePermissionPicker` | `src/vs/sessions/contrib/providers/copilotChatSessions/browser/permissionPicker.ts`, `mobilePermissionPicker.ts` | The agent window's **new-chat page** (`Menus.NewSessionControl`) | Sessions layer |
| `PermissionPickerActionItem` | `src/vs/workbench/contrib/chat/browser/widget/input/permissionPickerActionItem.ts` | The running **chat input toolbar** (`MenuId.ChatInputSecondary`) — both the editor window's chat and the agent window's running session | Workbench layer |
...
The `Menus.NewSessionRepositoryConfig` action (the generic per-property picker host, including the Claude new-session `permissionMode` fallback described above) is now hidden entirely for **quick chats** — workspace-less sessions created from the agent window's "Chats" section — via `IsQuickChatSessionContext.negate()` in its `when` clause. Quick chats have no repository/session-config surface to show, so the entire chip (not just individual properties) disappears rather than rendering empty. The `Menus.NewSessionControl` auto-approve/mode pickers are unaffected by this gate; they still apply to quick chats the same as any other new session.
...
- **2026-07-02** — f9f2fd558a — reconciliation: documented that `Menus.NewSessionRepositoryConfig` (the generic per-property picker host, including the Claude new-session `permissionMode` fallback chip) is now gated by `IsQuickChatSessionContext.negate()` and fully hidden for workspace-less quick chats (`e63efae792a`, "sessions: quick chats (workspace-less sessions) in the Agents window"), while `Menus.NewSessionControl` pickers are unaffected. No other body changes — the "Simplify agent sandbox settings" (`a68095a2946`), model-picker label wording (`6d9a8ec77c0`), and the Automations management UI (`4c959fa6747`) commits touched files this doc covers but only in ways that are already generalized as out-of-scope (sandbox toggle, unrelated label copy) or cosmetic (a stale `// "Pick Model"` comment).
