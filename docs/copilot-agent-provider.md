...
- Translating SDK elicitation callbacks into generic AHP input-request state, and honoring the host-level terminal override setting when deciding whether Agent Host shell tools replace the SDK defaults.
...
- **gotcha** (2026-04-20, copilotAgent.test.ts) - when renaming `CopilotAgent.id`, audit hardcoded literals in tests under `src/vs/platform/agentHost/test/node/`, including `protocol/copilotRealSdk.integrationTest.ts`. `AgentSession.uri('copilot', ...)` constructions are not type-checked, and the real-SDK file is gated on `AGENT_HOST_REAL_SDK=1`, so stale provider ids can sit broken indefinitely.
- **gotcha** (2026-04-21, updated 2026-05-28, package.json:@github/copilot) - the root `package.json` and `remote/package.json` versions of `@github/copilot` (the CLI) normally track `extensions/copilot/package.json`. The Copilot extension ships with VS Code and is exercised by Copilot's own validation, so its pinned version is the known-good baseline. **Exception** (first hit in `dced3b17d10`): when bumping `@github/copilot-sdk`, its declared `peerDependencies["@github/copilot"]` constraint may force the CLI ahead of the extension's pin (e.g. SDK `1.0.0-beta.8` required CLI `^1.0.55-1` while the extension was still at `1.0.49`). Follow the SDK peer in that case; the extension will catch up in its next bump. `@github/copilot-sdk` itself is not pinned by the extension and bumps independently.
- **gotcha** (2026-05-28, build/linux/debian/dep-lists.ts) — `@github/copilot` ships a prebuilt `prebuilds/linux-x64/runtime.node` (Bun's V8 isolate runtime). When the CLI bumps to a runtime built against a newer glibc tier, `dpkg-shlibdeps` emits a higher `libc6 (>= X.Y)` symbol-group floor in the deb's auto-detected deps, and the strict `FAIL_BUILD_FOR_NEW_DEPENDENCIES` allowlist in `build/linux/debian/dep-lists.ts` fails the **`vscode-linux-x64-prepare-deb`** Azure DevOps step (see [testing#debt--gotchas](./testing.md#debt--gotchas) — not exercised by GitHub Actions PR CI). Diagnose with `objdump -T <electron-build>/resources/app/extensions/copilot/dist/node_modules/@github/copilot/prebuilds/linux-x64/runtime.node | grep -oE 'GLIBC_[0-9.]+' | sort -u` and one-line bump the offending entry in `dep-lists.ts`. Bumping the floor in the deps list is a no-op for distro support because the overall `libc6` floor for the .deb stays at the highest entry (`>= 2.28`, pinned by other native modules) — apt resolves the package's union; this dep-list churn is just bookkeeping so the allowlist diff is zero. Same pattern previously bit `@vscode/sqlite3` bumps (`88eb80e805b`, opposite direction). Confirm only amd64 changed; arm64 GLIBC sets have been stable in copilot CLI bumps, and copilot does not ship armhf.
...
- **2026-05-15** — 12443ea83d — reconciliation: documented provider-owned customizations/completions, host terminal override behavior, and elicitation input requests from `cb855bd361c`, `1fbe0acb198`, `410e3ee6e09`, and `acdf223a800`; refreshed the provider-specific real-SDK test reference after `0d23db45a18`.
