...
- **gotcha** (2026-04-21, package.json:@github/copilot) — the root `package.json` and `remote/package.json` versions of `@github/copilot` should track `extensions/copilot/package.json`. The copilot extension ships with VS Code and is exercised by Copilot's own validation, so its pinned version is the known-good baseline. The SDK package's TS types (`@github/copilot-sdk`) and the bundled CLI server's runtime JSON schema (`@github/copilot/schemas/api.schema.json`) drift independently across `@github/copilot` releases; in particular, 1.0.34 makes `ModelCapabilities.limits` optional in the schema while the SDK `.d.ts` still declares it required, causing `_listModels` to throw `TypeError: Cannot read properties of undefined (reading 'max_context_window_tokens')` at runtime. Stay aligned with the extension's pin and bump in lockstep with it.
