...
This applies to lifecycle events that synthesize several AHP actions as well as direct message/tool events. In particular, live `skill.invoked` handling resolves its event-level `agentId` once and threads the resulting `parentToolCallId` through the synthesized start, ready, and complete actions. History replay performs the same lookup independently. Omitting the scope from any one action either puts the skill row in the root chat or sends a transition to a chat where the matching tool-call state does not exist.
...
- **gotcha** (2026-07-04, copilotAgentSession.ts:onSkillInvoked) — a subagent `skill.invoked` event has an event-level `agentId` but no usable `data.parentToolCallId`; resolve it through `_parentToolCallIdsByAgentId`, drop it when unmapped, and pass the same resolved `parentToolCallId` to all three synthesized tool-call actions. Emitting them without scope silently attributes the live skill invocation to the root chat while history replay attributes it correctly.
...
- **2026-07-04** — 3a9278fc17 — routed live `skill.invoked` synthesis through the existing event-level `agentId` to parent-tool-call map, including the unmapped-subagent guard, and added focused regression coverage that checks all three synthesized actions remain in the subagent scope while preserving the linked `SKILL.md` path.
