...
Required authentication is also checked synchronously before a turn starts. `AgentHostSessionHandler._ensureRequiredAuthentication` runs on both paths: the normal create-and-subscribe path and the eager/provisional-session path that already has hydrated session state. It delegates to `resolveAuthenticationInteractively` in `agentHostAuth.ts`, which first reuses an existing token; when none exists and an `ICommandService` is available, it directly invokes `workbench.action.chat.triggerSetup` with the standard **Sign in to use GitHub Copilot** account-options dialog (GitHub / Google / Apple / GHE), re-resolves the resulting token, forwards it with `authenticate`, and lets the original request continue. Cancellation returns `false`; provider/setup/transport errors throw unchanged so the request reports the real failure rather than replacing it with a generic auth-required message. Local and remote contributions provide their command service to the shared helper.

**Cache lifecycle rules** (all three must hold):

- **Reserve the token before the RPC and undo on failure.** `updateAndIsChanged` records the token before `authenticate()` so concurrent passes dedupe; if the RPC throws, call `cache.clear(resource, scopes)` so the next attempt is not suppressed.
...
- **gotcha** (2026-04-22, updated 2026-07-13, agentHostAuth.ts:AgentHostAuthTokenCache / resolveAuthenticationInteractively) — token dedupe records the resource/scope/token before the `authenticate()` RPC, then **must clear that exact entry on any throw**; otherwise a transient failure poisons the cache and suppresses retries. Clear the whole cache on `onAgentHostStart` because the process loses its in-memory auth state. Missing-token turn auth must go through the standard Copilot setup command before the raw provider `createSession` fallback, then re-resolve the token; do not drop signed-out users directly into GitHub's device-code subflow or swallow the setup/provider error into `false`.
