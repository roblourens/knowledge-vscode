...
The `AgentHostAuthTokenCache` class (`agentHostAuth.ts`) coordinates token forwarding per `(resource, normalized scopes)` key. `authenticate(...)` dedupes completed tokens, shares and awaits the same-token in-flight promise, serializes a different token behind current work, and records completion only after the Agent Host `authenticate` RPC succeeds. The same cache is used by background credential-change passes, interactive turn preflight, MCP authentication, and the smoke-test token path, so a turn cannot race ahead while provider infrastructure (for example Claude's proxy) is still starting.

Required authentication is also checked synchronously before a turn starts. `AgentHostSessionHandler._ensureRequiredAuthentication` runs on both paths: the normal create-and-subscribe path and the eager/provisional-session path that already has hydrated session state. It delegates to `resolveAuthenticationInteractively` in `agentHostAuth.ts`. Local and remote contributions invoke this orchestration helper through `IInstantiationService.invokeFunction`; it resolves `IAuthenticationService`, `ICommandService`, and `ILogService` from the `ServicesAccessor`, while its options contain only per-connection context (cache, log prefix, and `authenticate` callback).

Interactive auth first reuses an existing token. When none exists, it unconditionally invokes `workbench.action.chat.triggerSetup` with the standard **Sign in to use GitHub Copilot** account-options dialog (GitHub / Google / Apple / GHE), `disableChatViewReveal: true`, and a structured-result request. The no-command-service / raw provider `createSession` fallback was removed: every in-tree interactive caller uses the product setup flow. Setup cancellation returns `false`; provider/setup failure carries the original `Error` and whether setup already showed its retry UI, so Agent Host reports the real failure without moving focus from the originating chat or showing a second retry dialog. On success the helper re-resolves the token, forwards it, and lets the original request continue.

**Cache lifecycle rules** (all four must hold):

- **Completion means RPC success.** Never mark a token completed before `authenticate()` settles; failed authentication remains retryable.
- **In-flight work is part of dedupe.** Same-token callers await the shared promise. Different-token callers serialize behind current work and re-check state before starting.
- **Scoped clear is scoped.** `clear(resource, scopes)` increments that key's generation and cancels only pre-clear work for that key; it must not invalidate unrelated in-flight resources. `clear(resource)` applies the same rule to all keys for that resource.
- **Global clear invalidates every generation.** `clear()` is used on `onAgentHostStart` because the process loses its in-memory auth state; all pre-restart completed and queued work must be ignored.
...
9. **Is it local-only lifecycle** (restart, port wiring, dev mode)? → `IAgentHostService` and friends, *not* the handler.
...
### Reverse RPC: remote AH vs local AH

The agent host often needs to call back into the renderer to access client-side resources — most importantly, the client's `IFileService` for reading the synced customization bundle (`vscode-synced-customization://`). The two transports wire this reverse-RPC differently:

- **Remote AH (WebSocket):** AHP is JSON-RPC over WebSocket, which is bidirectional by design. The server can originate reverse requests freely; `ProtocolServerHandler` originates them and `RemoteAgentHostProtocolClient._handleReverseRequest` answers them. The `AHPFileSystemProvider` / `IRemoteFilesystemConnection` abstraction consumes the answers. No special IPC is needed.

- **Local AH (MessagePort utility process):** VS Code's `IPCClient ↔ UtilityProcessServer` IPC exposes only a one-way `ProxyChannel.fromService(IAgentService)` — there is no AHP layer to piggy-back on. However, `IPCClient` implements both `IChannelClient` AND `IChannelServer`, so reverse-RPC is achieved by registering a **separate server channel on the renderer**. The new channel is `AGENT_HOST_CLIENT_RESOURCE_CHANNEL` (defined in `src/vs/platform/agentHost/common/agentHostClientResourceChannel.ts`). The renderer registers it on the `MessagePortClient` with `clientId` as context. In `agentHostMain.ts`, `onDidAddConnection` / `onDidRemoveConnection` handlers (gated on `server instanceof UtilityProcessServer`) register per-connection authorities on the shared `AgentHostClientFileSystemProvider` instance, using `getChannel(name, c => c.ctx === clientId)` to reach the per-renderer channel. The channel's method shapes (`resourceList`, `resourceRead`, `resourceWrite`, `resourceDelete`, `resourceMove`) intentionally mirror AHP reverse-request shapes so the same `AHPFileSystemProvider` is used on both sides.
...
- **gotcha** (2026-05-26, agentHost.config.contribution.ts / agentHostStarter.config.contribution.ts) — `IConfigurationRegistry` is **per-process**: a `chat.agentHost.*` key registered only in `chat.shared.contribution.ts` is visible to the renderer's registry alone. The main process (e.g. `app.ts` calling `isAgentHostEnabled`) and the remote server (e.g. `nodeAgentHostStarter.ts` reading OTel keys) each see their own registry — un-registered keys make `getValue()` return `undefined`, silently ignoring the declared default. Today's split: `agentHost.config.contribution.ts` registers only `chat.agentHost.enabled` and is side-effect-imported by `electronAgentHostStarter.ts` (registration travels into the main process via `app.ts`'s import of the starter class); `agentHostStarter.config.contribution.ts` registers the seven starter-consumed keys (`claudeAgent.path` + 6 `otel.*`) and is side-effect-imported by both `electronAgentHostStarter.ts` and `nodeAgentHostStarter.ts`. The remote server intentionally does **not** register `chat.agentHost.enabled` — its spawn decision comes from `--agent-host-port` / `--agent-host-path` CLI args, not the workbench setting, so leaving the key un-registered there makes accidental future reads fail loud rather than silently picking up a workbench-only default. Keep the wiring co-located with the consumer file rather than at the process entry so it's discoverable from either end. **Note on the agent host process itself:** it does NOT read `chat.agentHost.*` via workbench `IConfigurationService`. The starter translates settings into env vars (`buildAgentHostOTelEnv`, `VSCODE_AGENT_HOST_CLAUDE_SDK_PATH`); runtime knobs flow through the agent-host's own root/session config service (`IAgentConfigurationService.getEffectiveValue`). Don't replicate workbench settings into the agent host process — extend the env-var translation or the host-side config service instead.
- **gotcha** (2026-05-01, electronAgentHostStarter.ts / nodeAgentHostStarter.ts) — the spawned local agent host process derives its `userDataPath` from `parseArgs(process.argv)` inside `agentHostMain.ts`, NOT from any Electron-level inheritance. The starters explicitly forward `--user-data-dir <parent userDataPath>` (alongside `--logsPath`) so that all host-owned state — `SessionDataService` (`{userData}/agentSessionData/`), `AgentPluginManager` (`{userData}/agentPlugins/`), and the root `agent-host-config.json` under `appSettingsHome` — lands inside the parent app's custom user-data dir. If you add a new starter or change how the local host is spawned, forward `--user-data-dir` the same way; without it the child silently falls back to the platform-default path for the build's quality and the parent's `--user-data-dir` is ignored.
...
- **gotcha** (2026-04-22, updated 2026-07-18, agentHostAuth.ts:AgentHostAuthTokenCache / resolveAuthenticationInteractively) — token dedupe must include the in-flight promise and mark completion only after RPC success; same-token callers await, different tokens serialize, and failed attempts stay retryable. Scoped clear uses per-key generations so it cannot cancel unrelated resources; global clear invalidates all pre-restart work. Interactive auth always uses the standard Copilot setup command, keeps the originating chat focused (`disableChatViewReveal`), distinguishes cancellation from failure, and propagates the original provider error. The orchestration helpers obtain DI-owned services from a `ServicesAccessor`; do not re-add services or an optional command service to the options bag.
...
- **debt** (2026-07-04, localAgentHostService.ts:_connect + node/agentHostService.ts:AgentHostProcessManager + agentHostChatContribution.ts) — the local agent host process is spawned **fully lazily**: the utility process only starts when something first accesses `IAgentHostService` (constructing `LocalAgentHostServiceClient`, whose ctor calls `_connect()` → `acquirePort('vscode:createAgentHostMessageChannel')` → the main-process `AgentHostProcessManager`'s once-listener on `starter.onRequestConnection` → `starter.start()`). Today the first access is `AgentHostContribution` at `WorkbenchPhase.AfterRestored`, so the (I/O-heavy) spawn + SDK load + auth only begins *after* window restore — one lifecycle phase later than the extension host (`NativeExtensionService` starts it at `LifecyclePhase.Ready`). Starting the agent host earlier (a prewarm contribution at `BlockRestore`, gated on `isAgentHostEnabled`, in both the agent and editor windows) would overlap the spawn with restore and get live sessions/openability ready sooner. Because `_connect()` is non-blocking, the prewarm adds no measurable restore cost. Tracked in [microsoft/vscode#324330](https://github.com/microsoft/vscode/issues/324330). (A prewarm + contribution-reorder implementation was prototyped and reverted to keep the session-caching PR focused; see `changes/2026-07-04-local-agent-host-session-caching/`.)

## Changelog

- **2026-07-18** — d38816526e — documented the review-hardened authentication design: promise-aware token forwarding with per-key/global generations, ServicesAccessor-owned orchestration, mandatory product setup for interactive auth, focus preservation, structured cancellation/failure, and duplicate-retry suppression. Squash-merged as [#325725](https://github.com/microsoft/vscode/pull/325725) (`efed051eed`).

- **2026-07-13** — 08093ec0e7 — documented turn-time authentication for both eager and normal session paths, the standard Copilot account-options setup dialog, token re-resolution/forwarding, direct error propagation, and the actual reserve-before-RPC/clear-on-failure cache behavior. PR [#325725](https://github.com/microsoft/vscode/pull/325725).

- **2026-07-04** — 49de3ac4ac — added a debt entry documenting the local agent host's **fully-lazy process spawn** (first `IAgentHostService` access → `LocalAgentHostServiceClient._connect` → `AgentHostProcessManager` once-listener → `starter.start()`), which happens at `WorkbenchPhase.AfterRestored`, one lifecycle phase later than the extension host at `LifecyclePhase.Ready`. Prewarming the process + reordering the sessions contribution is tracked in [#324330](https://github.com/microsoft/vscode/issues/324330); the shared startup session cache that mitigates the *persisted*-list half is in [agent-host-sessions-providers](./agent-host-sessions-providers.md#startup-session-cache-cross-window-persistence).
